...
\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
  \tikzstyle{every state}=[fill={rgb:yellow,1;white,3}]
    ...
    \path[->]
    (q_0) edge [bend left]  node {rcv(GETSERVER)}    (q_1)
    (q_1) edge [bend left]  node {snd(NOSERVER) | timeout}    (q_0)
          edge              node {snd(SERVERRES)}  (q_2);
\end{tikzpicture}
...
\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
  \tikzstyle{every state}=[fill={rgb:yellow,1;white,3}]
    ...
    \path[->]
    (q_0) edge [bend left]  node {rcv(REGISTER)} (q_1)
    (q_1) edge [bend left]  node {timeout}       (q_0)
          edge              node {snd(OK)}       (q_2);
\end{tikzpicture}

El servidor envía una petición de registro (REGISTER) al directorio. Este debe aceptarlo y responder que sí (OK). Si el mensaje se pierde por lo que sea, salta el timeout y se reintenta el registro.
...
\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
  \tikzstyle{every state}=[fill={rgb:yellow,1;white,3}]
    ...
    (q_1) edge              node {\tiny timeout | rcv(NOSERVER)}       (q_0)
          edge [bend left]  node {\tiny rcv(SERVERRES)}                (q_2)
    (q_2) edge [bend left]  node {\tiny connect (PRE\_CONNECTION)}     (q_3)
    (q_3) edge [bend left]  node {\tiny timeout}					     (q_2)
    	  edge              node {\tiny connected (PRE\_REGISTRATION)} (q_4)
    ...
    (q_9) edge [bend left]  node {\tiny snd(GET\_INFO)}				 (q_10)
    	  edge [loop above] node {\tiny snd(SEND <msg>)}				 ()
